...
SYSTEM_PROMPT = (
    ...
    "Generate a set of distinct persona types for all players in the game. "
    "Use only the game environment and rules provided. "
    "When uncertain, say 'unknown' or note low confidence."
)

USER_INSTRUCTIONS = (
    "Task: generate N distinct player 'types' for this PGG, including but not limited to incentives, "
    "strategies, beliefs, and personality. Ground every claim in plausible observed behavior for this "
    "environment, but DO NOT cite specific rounds, quotes, or numeric evidence. "
    "The generated types are intended to serve as prompts defining player types in new PGG environments. "
    "Use present tense throughout. Avoid redundancy; do not repeat the same trait or rationale across sections. "
    "Each type must be distinct from the others.\n"
    "Output format: JSON only. The JSON must have a top-level key 'types' with a list of N objects. "
    "Each object must have keys 'id' and 'text'. The 'text' value is a free-form type description, "
    "but you MUST include each header line listed under "
    "REQUIRED_HEADERS. Each required header must appear exactly once as its own line, followed by relevant details.\n"
...
def load_config_map(config_path):
...
def build_user_prompt(cfg, n_players):
    ...
    user_parts = [
        ...
        '{"types": [{"id":"type_1","text":"<TYPE_1_TEXT>"}, {"id":"type_2","text":"<TYPE_2_TEXT>"}, "..."]}',
    ]
    return "\n".join(user_parts)
