...
from .build_compact_observer_batch_inputs import (
    _contribution_rule,
    _empty_chat_index,
    _index_chat_log,
...
def main() -> None:
    ...
    parser.add_argument(
        "--learn-output",
        type=Path,
        default=Path(__file__).resolve().parents[1] / "Persona/transcripts_observer_compact_learn.jsonl",
    )
    parser.add_argument(
        "--val-output",
        type=Path,
        default=Path(__file__).resolve().parents[1] / "Persona/transcripts_observer_compact_val.jsonl",
    )
