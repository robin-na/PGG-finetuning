...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Parse SimBench individual-level batch outputs into single-choice labels."
    )
    ...
    for record in raw_rows:
        ...
        if custom_id not in manifest.index:
            ...
            option_labels = json.loads(str(manifest_row["option_labels_json"]))
            try:
                raw_text = extract_text_from_response_record(record)
                parsed_label, validation_errors = parse_choice_label(raw_text, option_labels)
            except Exception as exc:
                row_errors.append(str(exc))

        parse_success = not row_errors and not validation_errors and parsed_label is not None
        if parse_success:
        ...
        parsed_rows.append(
            {
                ...
                "simbench_row_id": str(manifest_row["simbench_row_id"]) if manifest_row is not None else "",
