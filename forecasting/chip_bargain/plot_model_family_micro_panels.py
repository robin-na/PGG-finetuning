...
METRICS = [
    ("player_final_surplus", "Player Final Surplus"),
    ("player_proposer_mean_trade_ratio", "Player Mean Trade Ratio"),
    ("player_proposer_acceptance_rate", "Player Acceptance Rate"),
    ("round_end_surplus_ratio", "Round-End Surplus Ratio"),
    ("round_mean_trade_ratio", "Round Mean Trade Ratio"),
    ("round_acceptance_rate", "Round Acceptance Rate"),
]
DEFAULT_BOOTSTRAP_ITERS = 300
DEFAULT_RNG_SEED = 29
DEFAULT_RANDOM_BASELINE_ITERS = 30
...
def _load_run_tables(run_name: str) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    return (
        ...
        pd.read_csv(run_dir / "generated_player_records.csv"),
        pd.read_csv(run_dir / "human_player_records.csv"),
        pd.read_csv(run_dir / "generated_round_records.csv"),
        pd.read_csv(run_dir / "human_round_records.csv"),
    )
...
def _treatment_scores_for_metric(
    ...
    *,
    ...
    if metric_name == "player_final_surplus":
        generated_metric_df = generated_player_df
        human_metric_df = human_player_df
        column_name = "final_surplus"
    elif metric_name == "player_proposer_mean_trade_ratio":
        generated_metric_df = generated_player_df
        human_metric_df = human_player_df
        column_name = "proposer_mean_trade_ratio"
    elif metric_name == "player_proposer_acceptance_rate":
        generated_metric_df = generated_player_df
        human_metric_df = human_player_df
        column_name = "proposer_acceptance_rate"
    elif metric_name == "round_end_surplus_ratio":
        generated_metric_df = generated_round_df
        human_metric_df = human_round_df
        column_name = "round_end_surplus_ratio"
    elif metric_name == "round_mean_trade_ratio":
        generated_metric_df = generated_round_df
        human_metric_df = human_round_df
        column_name = "round_mean_trade_ratio"
    elif metric_name == "round_acceptance_rate":
        generated_metric_df = generated_round_df
        human_metric_df = human_round_df
        column_name = "round_acceptance_rate"
    else:
...
def _shared_generated_count_map(run_tables: dict[str, tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]]) -> dict[str, int]:
...
def _resample_linked_records(
    *,
    ...
    sampled_games = game_df.sample(n=n_games, replace=True, random_state=int(rng.integers(2**31 - 1)))
    sampled_game_parts: list[pd.DataFrame] = []
    sampled_player_parts: list[pd.DataFrame] = []
    sampled_round_parts: list[pd.DataFrame] = []
    for sample_index, game_row in enumerate(sampled_games.to_dict(orient="records")):
        custom_id = str(game_row["custom_id"])
        sampled_game_parts.append(pd.DataFrame([{**game_row, "bootstrap_sample_index": sample_index}]))
        player_part = player_df[player_df["custom_id"] == custom_id].copy()
        round_part = round_df[round_df["custom_id"] == custom_id].copy()
        player_part["bootstrap_sample_index"] = sample_index
        round_part["bootstrap_sample_index"] = sample_index
        sampled_player_parts.append(player_part)
        sampled_round_parts.append(round_part)
    return (
        pd.concat(sampled_game_parts, ignore_index=True) if sampled_game_parts else pd.DataFrame(),
        pd.concat(sampled_player_parts, ignore_index=True) if sampled_player_parts else pd.DataFrame(),
        pd.concat(sampled_round_parts, ignore_index=True) if sampled_round_parts else pd.DataFrame(),
    )


def _split_linked_records(
    *,
    game_df: pd.DataFrame,
    player_df: pd.DataFrame,
    round_df: pd.DataFrame,
    ...
    sample_size = min(int(n_games), int(len(game_df) // 2))
    if sample_size <= 0:
        empty = pd.DataFrame()
        return empty, empty, empty, empty, empty, empty
    permutation = rng.permutation(len(game_df))
    lhs_games = game_df.iloc[permutation[:sample_size]].reset_index(drop=True)
    rhs_games = game_df.iloc[permutation[sample_size : 2 * sample_size]].reset_index(drop=True)
    lhs_ids = set(lhs_games["custom_id"].astype(str))
    rhs_ids = set(rhs_games["custom_id"].astype(str))
    lhs_players = player_df[player_df["custom_id"].astype(str).isin(lhs_ids)].reset_index(drop=True)
    rhs_players = player_df[player_df["custom_id"].astype(str).isin(rhs_ids)].reset_index(drop=True)
    lhs_rounds = round_df[round_df["custom_id"].astype(str).isin(lhs_ids)].reset_index(drop=True)
    rhs_rounds = round_df[round_df["custom_id"].astype(str).isin(rhs_ids)].reset_index(drop=True)
    return lhs_games, lhs_players, lhs_rounds, rhs_games, rhs_players, rhs_rounds
...
def _noise_ceiling_summary(
    ...
    *,
    ...
    human_games_by_treatment = {
        str(treatment_name): group.reset_index(drop=True)
        for treatment_name, group in human_game_df.groupby("treatment_name", sort=True)
    }
    human_player_by_treatment = {
        str(treatment_name): human_player_df[human_player_df["treatment_name"] == treatment_name].reset_index(drop=True)
        for treatment_name in human_games_by_treatment
    }
    human_round_by_treatment = {
        str(treatment_name): human_round_df[human_round_df["treatment_name"] == treatment_name].reset_index(drop=True)
        for treatment_name in human_games_by_treatment
    }
    ...
    for _ in range(bootstrap_iters):
        ...
        for treatment_name, generated_count in shared_generated_count_map.items():
            human_games = human_games_by_treatment[treatment_name]
            human_players = human_player_by_treatment[treatment_name]
            human_rounds = human_round_by_treatment[treatment_name]
            if ceiling_method == "split_half":
                (
                    pseudo_generated_games,
                    pseudo_generated_players,
                    pseudo_generated_rounds,
                    pseudo_human_games,
                    pseudo_human_players,
                    pseudo_human_rounds,
                ) = _split_linked_records(
                    game_df=human_games,
                    player_df=human_players,
                    round_df=human_rounds,
                    n_games=generated_count,
                    rng=rng,
                )
                if pseudo_generated_games.empty or pseudo_human_games.empty:
                    continue
            else:
                pseudo_generated_games, pseudo_generated_players, pseudo_generated_rounds = _resample_linked_records(
                    game_df=human_games,
                    player_df=human_players,
                    round_df=human_rounds,
                    n_games=generated_count,
                ...
                pseudo_human_games, pseudo_human_players, pseudo_human_rounds = _resample_linked_records(
                    game_df=human_games,
                    player_df=human_players,
                    round_df=human_rounds,
            ...
            scores_df = _treatment_scores_for_metric(
                metric_name,
                generated_player_df=pseudo_generated_players,
                human_player_df=pseudo_human_players,
                generated_round_df=pseudo_generated_rounds,
                human_round_df=pseudo_human_rounds,
            )
            if not scores_df.empty:
                treatment_scores.extend(scores_df["score"].tolist())
        if treatment_scores:
...
def _random_baseline_summary(
    ...
    *,
    ...
    for _ in range(random_baseline_iters):
        _, random_players, random_rounds = build_random_record_frames(
        ...
        scores_df = _treatment_scores_for_metric(
            metric_name,
            generated_player_df=random_players,
            human_player_df=human_player_df,
            generated_round_df=random_rounds,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot chip-bargain micro-level model-family comparison panels.")
    parser.add_argument("--run-suffix", type=str, default="")
    parser.add_argument("--output-stem", type=str, default="headline_model_family_micro_panels")
    parser.add_argument("--title", type=str, default="Chip Bargaining Micro-Level")
    ...
    family_tables: dict[str, dict[str, tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]]] = {}
    family_scores: dict[str, pd.DataFrame] = {}
    ...
    for family_name, spec in available_families.items():
        run_tables: dict[str, tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]] = {}
    ...
    fig, axes = plt.subplots(len(METRICS), ncols, figsize=(fig_width, 3.0 * len(METRICS)), constrained_layout=False)
    if ncols == 1:
        axes = np.asarray(axes).reshape(len(METRICS), 1)
        fig.subplots_adjust(top=0.87, bottom=0.08, left=0.18, right=0.98, hspace=0.4)
    else:
        fig.subplots_adjust(top=0.87, bottom=0.08, left=0.11, right=0.99, hspace=0.4, wspace=0.12)
    ...
    fig.suptitle(args.title, fontsize=14, y=0.99)
