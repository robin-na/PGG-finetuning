"""Plot PGG behavioral skew across persona sources."""
...
METRICS = [
    ("mean_contribution_rate", "Mean\ncontribution"),
    ("full_contribution_rate", "Full\ncontribution"),
    ("zero_contribution_rate", "Zero\ncontribution"),
    ("contribution_sd", "Contribution\nvariability"),
    ("messages_per_round", "Messages\nper round"),
    ("reward_given_round_rate", "Reward\ngiven"),
    ("punish_given_round_rate", "Punishment\ngiven"),
    ("punish_received_round_rate", "Punishment\nreceived"),
]
...
def _collect_rows(metadata_root: Path) -> list[dict[str, object]]:
    ...
    for run in RUNS:
        ...
        significance_rows = {
            row["metric"]: row
            for row in _read_csv(metadata_dir / "comprehensive_significance_checks.csv")
        }
        ...
        for metric, metric_label in METRICS:
            ...
            game_low = float(row["game_id_cluster_ci_low"])
            game_high = float(row["game_id_cluster_ci_high"])
            rows.append(
                {
                    "persona_source": str(run["label"]).replace("\n", " "),
                    "run": run["run"],
                    "metric": metric,
                    ...
                    "standardized_game_ci_low": game_low / sd if sd > 0 else float("nan"),
                    "standardized_game_ci_high": game_high / sd if sd > 0 else float("nan"),
                    "game_ci_excludes_zero": (game_low > 0 and game_high > 0)
                    or (game_low < 0 and game_high < 0),
                }
            )
    return rows
...
def _plot(rows: list[dict[str, object]], output_dir: Path, output_stem: str) -> None:
    ...
    matrix = [[float("nan") for _ in RUNS] for _ in METRICS]
    ci_marker = [[False for _ in RUNS] for _ in METRICS]
    for row in rows:
        ...
        ci_marker[y][x] = str(row["game_ci_excludes_zero"]).lower() == "true"

    finite_values = [abs(value) for series in matrix for value in series if not math.isnan(value)]
    vmax = max(0.75, min(1.75, max(finite_values) * 1.05 if finite_values else 1.0))
    ...
    ax.set_title("Public-goods behavioral skew across persona sources", pad=12)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-stem", default="figure_pgg_behavior_skew_across_personas")
