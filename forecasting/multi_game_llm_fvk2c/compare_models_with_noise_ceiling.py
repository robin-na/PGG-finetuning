...
def _bootstrap_noise_ceiling(
    *,
    ...
    generated_counts = (
        generated_sessions.groupby("treatment_name", sort=True)
        .size()
        .rename("generated_n")
        .reset_index()
    )
    ...
    for bootstrap_iter in range(bootstrap_iters):
        ...
        for treatment_name, human_group in human_sessions.groupby("treatment_name", sort=True):
        ...
        pseudo_generated_sessions = pd.concat(pseudo_generated_parts, ignore_index=True)
        pseudo_human_sessions = pd.concat(pseudo_human_parts, ignore_index=True)
        pseudo_generated_scenarios = explode_session_frame(
            pseudo_generated_sessions,
            target_column="gold_target",
            data_source="human_bootstrap_model",
        )
        pseudo_human_scenarios = explode_session_frame(
            pseudo_human_sessions,
            target_column="gold_target",
            data_source="human_bootstrap_human",
        )
        _, _, overall_df = compute_metric_tables(
            generated_sessions=pseudo_generated_sessions,
            human_sessions=pseudo_human_sessions,
            generated_scenarios=pseudo_generated_scenarios,
            human_scenarios=pseudo_human_scenarios,
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Bootstrap a human-vs-human noise ceiling for the multi-game benchmark."
    )
    ...
    if not model_overall.empty and not summary.empty:
        summary = summary.merge(
            model_overall.rename(columns={"mean_value": "model_score", "stderr": "model_stderr"}),
    ...
    write_json(
        args.output_dir / "manifest.json",
        {
            ...
            "bootstrap_iters": args.bootstrap_iters,
            "seed": args.seed,
            "sampling_unit": "subject_session_row_with_nested_scenarios",
        },
    )
