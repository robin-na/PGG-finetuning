...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Parse multi-game forecasting batch outputs into validated structured JSON."
    )
    ...
    for record in raw_rows:
        ...
        if custom_id not in manifest.index:
            ...
            treatment = str(manifest_row["Treatment"])
            num_scenarios = int(manifest_row["num_scenarios"])
            scenario_manifest = json.loads(str(manifest_row["scenario_manifest_json"]))
            try:
