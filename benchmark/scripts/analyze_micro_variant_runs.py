...
def find_latest_run_dir(variant_root: Path) -> Path:
    ...
    if not candidates:
        raise FileNotFoundError(
            f"No run directories with micro_behavior_eval.csv under {variant_root}"
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run micro behavior comparison analysis for a split by selecting the latest run "
    ...
    parser.add_argument(
        ...
        help="Comma-separated variant folders under micro_behavior_eval.",
    )
    parser.add_argument(
        "--labels",
        type=str,
    ...
    parser.add_argument(
        "--analysis-root",
        type=Path,
        default=Path("reports/benchmark/micro_behavior"),
    )
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Extra arg string passed to run_analysis.py. Can repeat.",
    )
...
def main() -> int:
    ...
    micro_root = runs_root / split_rel / "micro_behavior_eval"
    if not micro_root.exists():
        raise FileNotFoundError(f"Micro root not found: {micro_root}")
    ...
    for variant, label in zip(variants, labels):
        variant_root = micro_root / variant
    ...
    if len(selected_run_refs) < 2:
        raise ValueError("Need at least two variant runs to run comparison analysis.")

    analysis_run_id = args.analysis_run_id
    if not analysis_run_id:
        split_id = str(split_rel).replace("/", "__")
        analysis_run_id = f"{split_id}__variants_latest"

    cmd = [
        args.python,
        "Micro_behavior_eval/analysis/run_analysis.py",
        "--eval_root",
        str(micro_root),
        "--compare_run_ids",
