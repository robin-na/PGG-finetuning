...
def split_meta(root: Path, default_root: Path, ood_root: Path) -> Dict[str, str]:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--ood-root", type=Path, default=Path("benchmark/data_ood_splits"))
    parser.add_argument(
        "--include-default",
        action="store_true",
...
def main() -> int:
    ...
    for root in roots:
        info = split_meta(root, default_root=default_root, ood_root=ood_root)
