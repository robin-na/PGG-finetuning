from __future__ import annotations

import math
from typing import Any, Dict, List, Optional, Tuple

try:
    from .utils import as_bool
except ImportError:
    from utils import as_bool


JSON_STOP_SENTINEL = "<<END_JSON>>"


def system_header_lines(env: Dict[str, Any], demographics_text: str, include_reasoning: bool) -> List[str]:
    endow = int(env.get("CONFIG_endowment", 0) or 0)
    mult = env.get("CONFIG_multiplier", "Unknown")
    aon = as_bool(env.get("CONFIG_allOrNothing", False))
    contrib_mode = f"either 0 or {endow}" if aon else f"integer from 0 to {endow}"
    lines: List[str] = []
    lines.append("You are playing an online public goods game (PGG).")
    if demographics_text:
        lines.append(demographics_text)
    lines.append(
        f"Each round, you are given {endow} coins and need to decide how many coins to put into the shared pot "
        f"({contrib_mode})."
    )
    lines.append("You will not see others' choices before you decide.")
    lines.append(f"The pot is multiplied by {mult}x and split equally among all players.")
    lines.append("Your round payoff is: coins you kept + your equal share of the multiplied pot.")
    punish_on = as_bool(env.get("CONFIG_punishmentExists", False))
    reward_on = as_bool(env.get("CONFIG_rewardExists", False))
    if punish_on and reward_on:
        lines.append("After contributions are redistributed, players may punish or reward each other.")
    elif punish_on:
        lines.append("After contributions are redistributed, players may punish each other.")
    elif reward_on:
        lines.append("After contributions are redistributed, players may reward each other.")
    if as_bool(env.get("CONFIG_chat", False)):
        lines.append("You may send messages to the group chat.")
    return lines


def system_header_plain(env: Dict[str, Any], demographics_text: str, include_reasoning: bool) -> str:
    return "\n".join(system_header_lines(env, demographics_text, include_reasoning))


def build_openai_messages(system_text: str, history_chunks: List[str]) -> List[Dict[str, str]]:
    history = "\n".join(history_chunks)
    return [
        {"role": "system", "content": system_text},
        {"role": "user", "content": history},
    ]


def round_open(env: Dict[str, Any], r: int) -> str:
    if as_bool(env.get("CONFIG_showNRounds", False)):
        return f'<ROUND i="{r} of {int(env.get("CONFIG_numRounds", 0) or 0)}">'
    return f'<ROUND i="{r}">'


def round_info_line(env: Dict[str, Any]) -> str:
    endow = int(env.get("CONFIG_endowment", 0) or 0)
    aon = as_bool(env.get("CONFIG_allOrNothing", False))
    contrib_mode = f"either 0 or {endow}" if aon else f"integer from 0 to {endow}"
    default_prop = float(env.get("CONFIG_defaultContribProp", 0.0) or 0.0)
    if default_prop > 0.0:
        pre = (
            f"{endow} coins start in the shared pot and you can choose to take some for yourself. "
            f"How many coins would you like to leave in the pot ({contrib_mode})?"
        )
    else:
        pre = f"You have {endow} coins in your pocket. How much would you like to contribute ({contrib_mode})?"
    mult = env.get("CONFIG_multiplier", "Unknown")
    return f"QUESTION: {pre} The pot multiplier is {mult}x."


def chat_stage_line(env: Dict[str, Any]) -> str:
    return "QUESTION: You may send a message to the group chat. You may stay silent. Speak only when it helps."


def format_contrib_answer(val: Any) -> str:
    return f"You contributed: {val}"


def contrib_format_line(env: Dict[str, Any], include_reasoning: bool) -> str:
    contrib_hint = "Set 'contribution' to the amount left in the pot (your contribution)."
    if include_reasoning:
        reasoning_hint = "Provide reasons for your action, keep it brief."
        fmt = '{"stage":"contribution","reasoning":<string>,"contribution":<int>}'
    else:
        reasoning_hint = ""
        fmt = '{"stage":"contribution","contribution":<int>}'
    return (
        f"RULES: {contrib_hint} {reasoning_hint}\n"
        "FORMAT (JSON ONLY): "
        f"{fmt}\n"
        f"Immediately after the JSON object, output {JSON_STOP_SENTINEL} and nothing else.\n"
        "YOUR RESPONSE:"
    )


def _use_binary_targets(action_prompt_mode: str) -> bool:
    return str(action_prompt_mode or "binary_targets").strip().lower() != "legacy_units"


def actions_format_line(
    tag: str,
    include_reasoning: bool,
    action_prompt_mode: str = "binary_targets",
) -> str:
    if _use_binary_targets(action_prompt_mode):
        if tag == "PUNISHMENT":
            dict_hint = (
                "Use 'punish' as a list of avatars to punish once each; omit players you do not punish."
            )
            fmt = '{"stage":"actions","punish":[<avatar>,...]}'
            if include_reasoning:
                fmt = '{"stage":"actions","reasoning":<string>,"punish":[<avatar>,...]}'
        elif tag == "REWARD":
            dict_hint = (
                "Use 'reward' as a list of avatars to reward once each; omit players you do not reward."
            )
            fmt = '{"stage":"actions","reward":[<avatar>,...]}'
            if include_reasoning:
                fmt = '{"stage":"actions","reasoning":<string>,"reward":[<avatar>,...]}'
        else:
            dict_hint = (
                "Use 'punish' as a list of avatars to punish once each and 'reward' as a list of avatars "
                "to reward once each. A player must not appear in both lists."
            )
            fmt = '{"stage":"actions","punish":[<avatar>,...],"reward":[<avatar>,...]}'
            if include_reasoning:
                fmt = (
                    '{"stage":"actions","reasoning":<string>,"punish":[<avatar>,...],'
                    '"reward":[<avatar>,...]}'
                )
        reasoning_hint = "Provide reasons for your action, keep it brief." if include_reasoning else ""
        return (
            f"RULES: {dict_hint} {reasoning_hint}\n"
            "FORMAT (JSON ONLY): "
            f"{fmt}\n"
            f"Immediately after the JSON object, output {JSON_STOP_SENTINEL} and nothing else.\n"
            "YOUR RESPONSE:"
        )

    if tag == "PUNISHMENT":
        dict_hint = "Use 'actions' as a dict mapping avatar -> nonnegative punishment units; omit zeros."
    elif tag == "REWARD":
        dict_hint = "Use 'actions' as a dict mapping avatar -> nonnegative reward units; omit zeros."
    else:
        dict_hint = (
            "Use 'actions' as a dict mapping avatar -> integer units; omit zeros. "
            "Negative=punishment, positive=reward."
        )
    if include_reasoning:
        reasoning_hint = "Provide reasons for your action, keep it brief."
        fmt = '{"stage":"actions","reasoning":<string>,"actions":{...}}'
    else:
        reasoning_hint = ""
        fmt = '{"stage":"actions","actions":{...}}'
    return (
        f"RULES: {dict_hint} {reasoning_hint}\n"
        "FORMAT (JSON ONLY): "
        f"{fmt}\n"
        f"Immediately after the JSON object, output {JSON_STOP_SENTINEL} and nothing else.\n"
        "YOUR RESPONSE:"
    )


def chat_format_line(include_reasoning: bool) -> str:
    if include_reasoning:
        reasoning_hint = "Provide reasons for your action, keep it brief."
        fmt = '{"stage":"chat","reasoning":<string>,"chat":<string|null>}'
    else:
        reasoning_hint = ""
        fmt = '{"stage":"chat","chat":<string|null>}'
    return (
        "RULES: Set 'chat' to a short message, or null/empty string if silent. "
        f"{reasoning_hint}\n"
        "FORMAT (JSON ONLY): "
        f"{fmt}\n"
        f"Immediately after the JSON object, output {JSON_STOP_SENTINEL} and nothing else.\n"
        "YOUR RESPONSE:"
    )


def max_tokens_reminder_line(max_tokens: int) -> str:
    return f"MAX_TOKENS: You may generate at most {max_tokens} tokens."


def redist_line(total_contrib: int, multiplied: float, active_players: int) -> str:
    m_str = f"{multiplied:.1f}" if isinstance(multiplied, (int, float)) and not math.isnan(multiplied) else ""
    per = (multiplied / active_players) if active_players > 0 else float("nan")
    per_str = f"{per:.1f}" if isinstance(per, (int, float)) and not math.isnan(per) else "NA"
    return (
        f'<REDIST total_contrib="{total_contrib}" multiplied_contrib="{m_str}" '
        f'active_players="{active_players}" redistributed_each="{per_str}"/>'
    )


def peers_contributions_csv(roster: List[str], focal: str, contrib_math: Dict[str, int]) -> Tuple[str, List[str]]:
    peer_order = [av for av in roster if av != focal]
    parts = [f"{av}={contrib_math.get(av, 'NA')}" for av in peer_order]
    return ",".join(parts), peer_order


def mech_info(env: Dict[str, Any], action_prompt_mode: str = "binary_targets") -> Optional[str]:
    reward_on = as_bool(env.get("CONFIG_rewardExists", False))
    punish_on = as_bool(env.get("CONFIG_punishmentExists", False))
    if not (reward_on or punish_on):
        return None
    if _use_binary_targets(action_prompt_mode):
        if reward_on and punish_on:
            return (
                "QUESTION: If you reward a player, each selected player costs you "
                f"{env.get('CONFIG_rewardCost', 0)} coins and gives that player "
                f"{env.get('CONFIG_rewardMagnitude', 0)} coins. If you punish a player, each selected player costs you "
                f"{env.get('CONFIG_punishmentCost', 0)} coins and deducts "
                f"{env.get('CONFIG_punishmentMagnitude', 0)} coins from that player. "
                "List which players to punish and which players to reward. Each selected player receives exactly one unit."
            )
        if reward_on:
            return (
                "QUESTION: If you reward a player, each selected player costs you "
                f"{env.get('CONFIG_rewardCost', 0)} coins and gives that player "
                f"{env.get('CONFIG_rewardMagnitude', 0)} coins. "
                "List which players to reward. Each selected player receives exactly one unit."
            )
        return (
            "QUESTION: If you punish a player, each selected player costs you "
            f"{env.get('CONFIG_punishmentCost', 0)} coins and deducts "
            f"{env.get('CONFIG_punishmentMagnitude', 0)} coins from that player. "
            "List which players to punish. Each selected player receives exactly one unit."
        )
    if reward_on and punish_on:
        return (
            "QUESTION: It will cost you, per reward unit, "
            f"{env.get('CONFIG_rewardCost', 0)} coins to give a reward of {env.get('CONFIG_rewardMagnitude', 0)} coins. "
            f"It will cost you, per punishment unit, {env.get('CONFIG_punishmentCost', 0)} coins to impose a deduction "
            f"of {env.get('CONFIG_punishmentMagnitude', 0)} coins. "
            "Who would you like to punish or reward, and by how many units?"
        )
    if reward_on:
        return (
            "QUESTION: It will cost you, per unit, "
            f"{env.get('CONFIG_rewardCost', 0)} coins to give a reward of {env.get('CONFIG_rewardMagnitude', 0)} coins. "
            "Who would you like to reward and by how many units?"
        )
    return (
        "QUESTION: It will cost you, per unit, "
        f"{env.get('CONFIG_punishmentCost', 0)} coins to impose a deduction of {env.get('CONFIG_punishmentMagnitude', 0)} coins. "
        "Who would you like to punish and by how many units?"
    )


def actions_tag(env: Dict[str, Any]) -> Optional[str]:
    reward_on = as_bool(env.get("CONFIG_rewardExists", False))
    punish_on = as_bool(env.get("CONFIG_punishmentExists", False))
    if punish_on and reward_on:
        return "PUNISHMENT/REWARD"
    if punish_on:
        return "PUNISHMENT"
    if reward_on:
        return "REWARD"
    return None
